...
if($_GET['a']=='FotoCek2'):
	if($_GET['id']!=''):
		$deger = intval( $_GET[id] );
		$res = $dw->getResults('SELECT * FROM galeriresimler where urunid = "'.$deger.'" or sessionID = "'.session_id().'" order by sira asc ');
	else:
		$res = $dw->getResults('SELECT * FROM galeriresimler where sessionID = "'.session_id().'" order by sira asc ');
	endif;
	$total_records = $dw->numRows();
	if ($total_records == "0") {
		echo '<li style="list-style:none;width:100%;"><div class="alert alert-danger"><strong>Bilgilendirme!</strong><br>Ürüne ait görsel bulunamadı.<br>Görsel eklemek için yukarıdan görsel seçin.</div></li>';
	...
	?>
		<li id="item-<?php echo $read->id;?>" style="margin:3px;list-style:none;float:left;border:1px dashed #c0c0c0;padding:10px;cursor:move;width:16.2%;float:left;">
			<div style="width: 100%;float:left;text-align: center;">
				<img src="../yukleme/galeri/<?php echo $read->resim;?>" width="100%" />
...
if($_GET['a']=='ResimDuzenSil2') {
	...
	$kontrol = $dw->getRow('SELECT * FROM galeriresimler WHERE id = ?', array('bindValues' => array($deger)));

	@unlink("../yukleme/galeri/".$kontrol->kucukresim);	
	@unlink("../yukleme/galeri/".$kontrol->ortaresim);	
	@unlink("../yukleme/galeri/".$kontrol->orta2resim);	
	@unlink("../yukleme/galeri/".$kontrol->resim);

	$sil = $dw->delete('galeriresimler', array('id = ?'), array($deger), array('configKey' => 'secondaryDB'));
...
if($_GET['a']=='FotoResimSirala2'):
	...
	foreach ($_POST['item'] as $position => $item) :
		...
		$guncelle = $dw->update('galeriresimler', $user, array('id = ?'), array($item), array('configKey' => 'secondaryDB'));
...
if($_GET['a']=='FotoCek3'):
	if($_GET['id']!=''):
		$deger = intval( $_GET[id] );
		$res = $dw->getResults('SELECT * FROM referansresimler where urunid = "'.$deger.'" or sessionID = "'.session_id().'" order by sira asc ');
	else:
		$res = $dw->getResults('SELECT * FROM referansresimler where sessionID = "'.session_id().'" order by sira asc ');
	endif;
	$total_records = $dw->numRows();
	if ($total_records == "0") {
		echo '<li style="list-style:none;width:100%;"><div class="alert alert-danger"><strong>Bilgilendirme!</strong><br>Ürüne ait görsel bulunamadı.<br>Görsel eklemek için yukarıdan görsel seçin.</div></li>';
	...
	?>
		<li id="item-<?php echo $read->id;?>" style="margin:3px;list-style:none;float:left;border:1px dashed #c0c0c0;padding:10px;cursor:move;width:16.2%;float:left;">
			<div style="width: 100%;float:left;text-align: center;">
				...
				<a href="javascript:void(0);" onclick="ResimDuzenSil3(<?php echo $read->id;?>)" class="btn btn-icon btn-danger" style="margin-top:3px;"><i class="fa fa-trash-o" style="margin-right:0px;"></i></a>
			</div>
		</li>
	<?php
		}
...
if($_GET['a']=='ResimDuzenSil3') {
	...
	$kontrol = $dw->getRow('SELECT * FROM referansresimler WHERE id = ?', array('bindValues' => array($deger)));
	...
	$sil = $dw->delete('referansresimler', array('id = ?'), array($deger), array('configKey' => 'secondaryDB'));
...
if($_GET['a']=='FotoResimSirala3'):
	...
	foreach ($_POST['item'] as $position => $item) :
		...
		$guncelle = $dw->update('referansresimler', $user, array('id = ?'), array($item), array('configKey' => 'secondaryDB'));
