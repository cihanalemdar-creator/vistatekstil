...
class kgPager {
    ...
    function page_links ($inactive_page_tag, $pager_url_last) {
        ...
        for ($i = $loop_start; $i <= $loop_finish; $i++) {
            if ($i == $this -> current_page) {
                $this -> page_links .= '<li '.$inactive_page_tag.' class="active"><a href="#">'.$i.'</a></li>';
            }else{
                $this -> page_links .= '<li class=""><a href="'.$this -> url.$i.$pager_url_last.'">'.$i.'</a></li>';
            }
        }
    }
    ...
    function previous_page ($previous_page_text, $pager_url_last) {
        if ($this -> current_page > 1) {
            $this -> previous_page = '<li class=""><a href="'.$this -> url.($this -> current_page - 1).$pager_url_last.'">'.$previous_page_text.'</a></li>';
        }
    }
    ...
    function next_page ($next_page_text, $pager_url_last) {
        if ($this -> current_page < $this -> total_pages) {
            $this -> next_page = '<li class=""><a href="'.$this -> url.($this -> current_page + 1).$pager_url_last.'">'.$next_page_text.'</a></li>';
        }
    }
    ...
    function first_page ($first_page_text, $pager_url_last) {
        if ($this -> current_page > 1) {
            $this -> first_page = '<li class=""><a href="'.$this -> url.'1'.$pager_url_last.'">'.$first_page_text.'</a></li>'; // :)
        }
    }
    ...
    function last_page ($last_page_text, $pager_url_last) {
        if ($this -> current_page < $this -> total_pages) {
            $this -> last_page = '<li class=""><a href="'.$this -> url.$this -> total_pages.$pager_url_last.'">'.$last_page_text.'</a></li>';
        }
    }
