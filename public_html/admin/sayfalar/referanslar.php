        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h5>Koleksiyon Ayarları</h5><!--Ekle Butonu oluşturuluyor.-->
                </div>
                <div class="ibox-content">
                    ...
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                <th width="90%">Koleksiyon Başlık</th>
                        ...
                        <tbody id="sortable">
                        <?php
                        $sorgu = "SELECT * FROM referanslar order by id desc ";
                        $list1 = $dw->getResults(''.$sorgu.'');
                        $total_records = $dw->numRows();
                        $pager_url = '?sayfa=referanslar&page='; 
                        $scroll_page = 5; 
                        $per_page = 10; 
                        ...
                        ?>
                            <tr id="item-<?php echo $row->id;?>">
                                <td style="vertical-align: middle;">Koleksiyonu düzenlemek için sağ kısımdaki düzenle butonunu kullanınız</td>
                                <td style="vertical-align: middle;" class="tooltip-demo">
									<a href="?sayfa=referanslar&islem=duzenle1&id=<?php echo $row->id; ?>" class="btn btn-sm waves-effect btn-default m-b-5" data-toggle="tooltip" data-placement="top" title="Düzenle"><i class="fa fa-edit"></i></a>
								</td>
                            </tr>
                        <?
                        }
...
$(document).ready(function(){
    $.ajax({
        type: 'POST', 
        url:'islem.php?a=FotoCek3&id=<?php echo $id;?>',
        data: '',       
...
$(document).ready(function(){ 
    ...
    $(function() {
        $("ul#resim").sortable({ opacity: 0.6, cursor: 'move', update: function() {
            ...
            $.post("islem.php?a=FotoResimSirala3", order, function(theResponse){
...
////////////////////////////////////////////////////////
function ResimDuzenSil3(S) {
    $.ajax({
        type: 'GET', 
        url:'islem.php?a=ResimDuzenSil3&id='+S,
        data: '',       
    ...
    <div class="row">
        <form action="index.php?sayfa=referanslar&islem=eklendi" method="post" enctype="multipart/form-data">
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
						<div class="row">						
                            <div class="col-sm-3">
                                ...
                                <label class="control-label">Koleksiyon Adı TR</label>
                                    <input type="text" name="baslik" id="baslik" class="form-control" required>
                                </div>
                            </div>
							<div class="col-sm-3">
                                <div class="form-group">
                                <label class="control-label">Koleksiyon Adı EN</label>
						...
						<div class="row">
							...
							<div class="col-sm-4">
								<div class="form-group">
								<label class="control-label">Koleksiyon Kapak Resmi</label>
...
$upload->image_x = 915;
$upload->image_y = 540;
$upload->process("../yukleme/referanslar");
...
$YoneticiID = $dw->insert('referanslar', $veri, array('configKey' => 'secondaryDB'));
...
$SID = $Oturum;
$guncelle = $dw->update('referansresimler', $user, array('sessionID = ?'), array($SID), array('configKey' => 'secondaryDB')); 

?>
<div class="alert alert-success">Bilgiler Eklendi!<br>Bilgiler başarıyla eklendi. Lütfen bekleyin yönlendiriliyorsunuz.</div>
<meta http-equiv="refresh" content="2; URL=index.php?sayfa=referanslar">

<? 
}else if ( $islem =="duzenle1" )  {
$id = intval( $_GET[id] );    

$veri = $dw->getRow('SELECT * FROM referanslar WHERE id = ?', array('bindValues' => array($id)));
?>
...
//* Resimler
  ...
  <form action="index.php?sayfa=referanslar&islem=duzenlendi1" method="post" enctype="multipart/form-data">
    <div class="row">
						...
						<div class="row">
							...
							<div class="col-sm-2">
								...
								<div class="form-group">
									...
									<img width="250px" height="250px" src="../yukleme/referanslar/<?php echo $veri->resim; ?>">
								</div>
								<div class="form-group">
									<a class="btn btn-danger" href="index.php?sayfa=referanslar&islem=resimsil1&id=<?php echo $veri->id;?>" onclick="return confirm('Resmi silmek istediğinize emin misiniz?');">Resmi Sil</a>
								</div>
							</div>
							<?php  } else { ?> 
...
$guncelle = $dw->update('referanslar', $veri, array('id = ?'), array($id), array('configKey' => 'secondaryDB'));
...
$kontrol = $dw->getRow('SELECT * FROM referansresimler WHERE urunid = ?', array('bindValues' => array($id)));
$varmi = $dw->numRows(); 
...
$sguncelle = $dw->update('referanslar', $gveri, array('id = ?'), array($id), array('configKey' => 'secondaryDB'));   
}
...
$id = intval( $_GET[id] );
$kategoriveri = $dw->getRow('SELECT * FROM referanslar WHERE id = ?', array('bindValues' => array($id)));

$veri = array(
   'resim' => ""
);
$sil = '../yukleme/referanslar/'.$kategoriveri->resim;
@unlink($sil);  
$guncelle = $dw->update('referanslar', $veri, array('id = ?'), array($id), array('configKey' => 'secondaryDB'));
?>
<div class="alert alert-success">Bilgiler Güncellendi!<br>Bilgiler başarıyla güncellendi. Lütfen bekleyin yönlendiriliyorsunuz.</div>
<meta http-equiv="refresh" content="2; URL=index.php?sayfa=referanslar&islem=duzenle1&id=<?php echo $id; ?>">
...
$kategoriveri = $dw->getRow('SELECT * FROM referanslar WHERE id = ?', array('bindValues' => array($id)));
$resimsil = '../yukleme/referanslar/'.$kategoriveri->resim;
 @unlink($resimsil); 
$sil = $dw->delete('referanslar', array('id = ?'), array($id), array('configKey' => 'secondaryDB'));

          $rsorgu = "Select * from referansresimler WHERE urunid='$id'";
          $rlist = $dw->getResults(''.$rsorgu.'');
          foreach($rlist as $rrow){
            $resimsil2 = '../yukleme/referanslar/'.$rrow->resim;
            @unlink($resimsil2); 
            } 
$sil2 = $dw->delete('referansresimler', array('urunid = ?'), array($id), array('configKey' => 'secondaryDB'));
?>
