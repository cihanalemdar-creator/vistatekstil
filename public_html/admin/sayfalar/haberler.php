        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h5>Haber Ayarları</h5><!--Ekle Butonu oluşturuluyor.-->
                    <a href="index.php?sayfa=haberler&islem=ekle" class="btn btn-success dim pull-right" style="margin-top: -8px;"><i class="fa fa-plus" aria-hidden="true"></i></a>
                </div>
                        ...
                        ?>
                            <tr id="item-<?php echo $row->id;?>">
                                ...
                                <td style="vertical-align: middle;" class="tooltip-demo">
                                     <a href="?sayfa=haberler&islem=duzenle1&id=<?php echo $row->id; ?>" class="btn btn-sm waves-effect btn-default m-b-5" data-toggle="tooltip" data-placement="top" title="Düzenle"><i class="fa fa-edit"></i></a>
                                   <a href="?sayfa=haberler&islem=sil&id=<?php echo $row->id; ?>" class="btn btn-sm waves-effect btn-default m-b-5" onclick="return confirm('Bilgileri silmek istediğinize emin misiniz?');" data-toggle="tooltip" data-placement="top" title="Sil"><i class="fa fa-times"></i></a><!--Sil butonu oluşturuluyor-->
                                </td>
                            </tr>
                        <?
                        }
...
function ResimDuzenSil2(S) {
    ...
    <div class="row">
        <form action="index.php?sayfa=haberler&islem=eklendi" method="post" enctype="multipart/form-data">
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h5>Haber Ekle</h5>
                ...
                <div class="ibox-content">
						<div class="row">
                            ...
                            <div class="col-sm-6">
                                ...
                                <label class="control-label">Haber Adı TR</label>
							...
							<div class="col-sm-6">
                                <div class="form-group">
                                <label class="control-label">Haber Adı EN</label>
							...
							<div class="col-sm-6">
                                <div class="form-group">
                                <label class="control-label">Haber Adı AR</label>
                        ...
                        <div class="row">
							...
							<div class="col-sm-6">
								<div class="form-group">
								<label class="control-label">Haber Kapak Resmi</label>
...
$upload->image_x = 360;
$upload->image_y = 216;
$upload->process("../yukleme/haberler");
...
$YoneticiID = $dw->insert('haberler', $veri, array('configKey' => 'secondaryDB'));
...
<meta http-equiv="refresh" content="2; URL=index.php?sayfa=haberler">

<? 
}else if ( $islem =="duzenle1" )  {
$id = intval( $_GET[id] );    

$veri = $dw->getRow('SELECT * FROM haberler WHERE id = ?', array('bindValues' => array($id)));
?>
...
//* Resimler
  ...
  <form action="index.php?sayfa=haberler&islem=duzenlendi1" method="post" enctype="multipart/form-data">
    <div class="row">
        ...
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
                    <div class="row">
							...
							<div class="col-sm-12">
								...
								<div class="form-group">
									...
									<img width="360px" height="216px" src="../yukleme/haberler/<?php echo $veri->resim; ?>">
								</div>
								<div class="form-group">
									<a class="btn btn-danger" href="index.php?sayfa=haberler&islem=resimsil1&id=<?php echo $veri->id;?>" onclick="return confirm('Resmi silmek istediğinize emin misiniz?');">Resmi Sil</a>
								</div>
							</div>
							<?php  } else { ?> 
					...
					<div class="row"> 
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <label class="control-label">Haber Detay TR</label>
							...
							<div class="col-sm-6">
                                <div class="form-group">
                                    <label class="control-label">Haber Detay EN</label>
							...
							<div class="col-sm-6">
                                <div class="form-group">
                                    <label class="control-label">Haber Detay AR</label>
...
$guncelle = $dw->update('haberler', $veri, array('id = ?'), array($id), array('configKey' => 'secondaryDB'));
...
$sguncelle = $dw->update('haberler', $gveri, array('id = ?'), array($id), array('configKey' => 'secondaryDB'));   
}
...
$id = intval( $_GET[id] );
$kategoriveri = $dw->getRow('SELECT * FROM haberler WHERE id = ?', array('bindValues' => array($id)));

$veri = array(
   'resim' => ""
);
$sil = '../yukleme/haberler/'.$kategoriveri->resim;
@unlink($sil);  
$guncelle = $dw->update('haberler', $veri, array('id = ?'), array($id), array('configKey' => 'secondaryDB'));
?>
<div class="alert alert-success">Bilgiler Güncellendi!<br>Bilgiler başarıyla güncellendi. Lütfen bekleyin yönlendiriliyorsunuz.</div>
<meta http-equiv="refresh" content="2; URL=index.php?sayfa=haberler&islem=duzenle1&id=<?php echo $id; ?>">
...
$kategoriveri = $dw->getRow('SELECT * FROM haberler WHERE id = ?', array('bindValues' => array($id)));
$resimsil = '../yukleme/haberler/'.$kategoriveri->resim;
 @unlink($resimsil); 
$sil = $dw->delete('haberler', array('id = ?'), array($id), array('configKey' => 'secondaryDB'));

          $rsorgu = "Select * from projeresimler WHERE urunid='$id'";
          $rlist = $dw->getResults(''.$rsorgu.'');
          foreach($rlist as $rrow){
            $resimsil2 = '../yukleme/haberler/'.$rrow->resim;
            @unlink($resimsil2); 
            } 
