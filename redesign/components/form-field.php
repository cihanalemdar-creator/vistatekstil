...
<div class="form-field<?= $fullWidth ? ' form-field--full' : '' ?><?= $error !== null ? ' has-error' : '' ?>">
    ...
    <?php elseif ($type === 'textarea'): ?>
        <textarea id="<?= e($name) ?>" name="<?= e($name) ?>" rows="<?= e($field['rows'] ?? 6) ?>"<?= $required ? ' required' : '' ?><?= $describedBy !== null ? ' aria-describedby="' . e($describedBy) . '"' : '' ?><?= $error !== null ? ' aria-invalid="true"' : '' ?>><?= e($fieldValue) ?></textarea>
    <?php else: ?>
    ...
    <?php endif; ?>
    <?php if ($required || $error !== null): ?><small class="field-error<?= $error !== null ? ' is-visible' : '' ?>" id="<?= e($errorId) ?>"><?= e($error ?? $labels['required']) ?></small><?php endif; ?>
    <?php if (isset($field['help'])): ?><small><?= e(is_array($field['help']) ? ($field['help'][$locale] ?? $field['help']['en']) : $field['help']) ?></small><?php endif; ?>
</div>
